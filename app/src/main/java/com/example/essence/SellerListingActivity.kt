...
class SellerListingActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        fetchAllSellerProperties()
        setupBottomNavigation()
    }

    // Updated: fetch all properties listed by this seller, no status filter
    private fun fetchAllSellerProperties() {
        ...
        Log.d(TAG, "Fetching ALL properties for user ID: $currentUserId using field '$SELLER_ID_FIELD_IN_PROPERTIES_COLLECTION'")
        firestore.collection("properties")
            .whereEqualTo(SELLER_ID_FIELD_IN_PROPERTIES_COLLECTION, currentUserId)
            .get()
            .addOnSuccessListener { documents ->
                if(documents.isEmpty){
                    Log.d(TAG, "No properties found for seller.")
                    Toast.makeText(this, "No property listings found.", Toast.LENGTH_SHORT).show()
                }
                propertyList.clear()
                ...
                Log.d(TAG, "Fetched ${propertyList.size} properties for seller.")
            }
            .addOnFailureListener { e ->
                Log.e(TAG, "Error fetching seller properties: ", e)
